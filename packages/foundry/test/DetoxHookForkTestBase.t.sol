...
import {PublicRPCURL} from "../script/PublicRPCURL.sol";
...
abstract contract DetoxHookForkTestBase is Test {
    ...
    function _initializeContracts() internal {
        ...
        address modifyLiquidityAddr = ChainAddresses.getPoolModifyLiquidityTest(CHAIN_ID);
        
        // Verify contracts exist
        require(poolManagerAddr.code.length > 0, "PoolManager not found");
        require(swapRouterAddr.code.length > 0, "SwapRouter not found");
        require(modifyLiquidityAddr.code.length > 0, "ModifyLiquidityRouter not found");
    ...
    function _deployDetoxHookWithHookMiner() internal {
        ...
        require(pythOracle != address(0), "Pyth oracle not available on this chain");
    ...
    function _configurePriceRegistry() internal {
        ...
        if (Currency.unwrap(currency0) < Currency.unwrap(currency1)) {
            ...
            if (token0.decimals() == 18) {
                ...
                mockRegistry.setPriceMapping(Currency.unwrap(currency0), ethPriceId, "WETH");
                mockRegistry.setPriceMapping(Currency.unwrap(currency1), usdcPriceId, "USDC");
            } else {
                // currency0 is USDC (6 decimals)
                mockRegistry.setPriceMapping(Currency.unwrap(currency0), usdcPriceId, "USDC");
                mockRegistry.setPriceMapping(Currency.unwrap(currency1), ethPriceId, "WETH");
            }
        }
    ...
    function _getRPCWithFailover(uint256 chainId) internal view returns (string memory) {
        ...
        string memory backupEnvVarName = PublicRPCURL.getBackupEnvVarName(chainId);
        try vm.envString(backupEnvVarName) returns (string memory backupEnvRPC) {
    ...
    function _getChainInfo() internal view returns (PublicRPCURL.ChainInfo memory) {
...
contract MockPriceRegistry {
    ...
    function setPriceMapping(address token, bytes32 priceId, string memory symbol) external {
