...
contract DetoxHookV2Test is Test, Deployers {
    ...
    bytes32 constant TOK1_PRICE_ID =
        0x1111111111111111111111111111111111111111111111111111111111111111;
    bytes32 constant TOK2_PRICE_ID =
    ...
    function formatETH(uint256 amount) internal pure returns (string memory) {
        ...
        if (decimals == 0) {
            ...
            return
                string(
                    abi.encodePacked(
                        _uintToString(ethAmount),
                        ".",
                        _uintToString(decimals),
                        " ETH"
                    )
                );
        }
    ...
    function formatUSDC(uint256 amount) internal pure returns (string memory) {
        ...
        if (decimals == 0) {
            return
                string(abi.encodePacked(_uintToString(usdcAmount), ".0 USDC"));
        } else {
            return
                string(
                    abi.encodePacked(
                        _uintToString(usdcAmount),
                        ".",
                        _uintToString(decimals),
                        " USDC"
                    )
    ...
    function _deployDetoxHookV2WithCreate2() internal {
        ...
        require(
            address(hook.priceRegistry()) == address(priceRegistry),
            "Hook not connected to price registry"
        );
        require(
            address(hook.pythOracle()) == address(mockOracle),
            "Hook not connected to oracle"
        );
    ...
    function _setupRealisticPool() internal {
        ...
        if (address(realETH) < address(realUSDC)) {
            ...
            console.log(
                "[OK] Currency ordering: ETH (currency0), USDC (currency1)"
            );
        } else {
            realisticCurrency0 = usdcCurrency; // USDC
            realisticCurrency1 = ethCurrency; // ETH
            console.log(
                "[OK] Currency ordering: USDC (currency0), ETH (currency1)"
            );
        }
        ...
        console.log(
            "[OK] Added liquidity - USDC:",
            LIQUIDITY_USDC_AMOUNT / 1e6
        );
    ...
    function test_SetupValidation() public {
        ...
        assertTrue(
            address(priceRegistry) != address(0),
            "PriceRegistry should be deployed"
        );
        assertTrue(
            address(mockOracle) != address(0),
            "MockOracle should be deployed"
        );

        // Verify pool initialization
        assertTrue(
            PoolId.unwrap(simplePoolId) != bytes32(0),
            "Simple pool should be initialized"
        );
        assertTrue(
            PoolId.unwrap(realisticPoolId) != bytes32(0),
            "Realistic pool should be initialized"
        );
        ...
        assertTrue(
            ...
            "Currency0 should map to ETH price ID"
        );
    ...
    function test_NoArbitrageWhenOracleMatchesPool() public {
        ...
        console.log(
            "Pool: 1:1 ratio, Oracle: 1:1 ratio -> No arbitrage expected"
        );
        ...
        for (uint i = 0; i < 2; i++) {
            ...
            console.log(
                "Testing direction:",
                zeroForOne ? "currency0->currency1" : "currency1->currency0"
            );

            // Record balances before
            uint256 hookBalance0Before = MockERC20(Currency.unwrap(currency0))
                .balanceOf(address(hook));
            uint256 hookBalance1Before = MockERC20(Currency.unwrap(currency1))
                .balanceOf(address(hook));
            ...
            uint256 hookBalance0After = MockERC20(Currency.unwrap(currency0))
                .balanceOf(address(hook));
            uint256 hookBalance1After = MockERC20(Currency.unwrap(currency1))
                .balanceOf(address(hook));

            assertEq(
                hookBalance0After,
                hookBalance0Before,
                "Hook should not capture currency0 when no arbitrage"
            );
            assertEq(
                hookBalance1After,
                hookBalance1Before,
                "Hook should not capture currency1 when no arbitrage"
            );
        }
    ...
    function test_ArbitrageWhenPoolOverpays() public {
        ...
        console.log(
            "Pool: 1:1 ratio, Oracle: currency0=$3000, currency1=$1000 -> Pool overpaying"
        );
        ...
        console.log(
            "Testing zeroForOne: currency0->currency1 (should capture arbitrage)"
        );
        ...
        console.log("Gas used for arbitrage-capturing swap:", gasUsed);
        ...
        for (uint i = 0; i < logs.length; i++) {
            if (
                logs[i].topics[0] ==
                keccak256(
                    "ArbitrageCaptured(bytes32,address,uint256,uint256,bool)"
                )
            ) {
        ...
        assertTrue(
            arbitrageCaptured,
            "ArbitrageCaptured event should be emitted"
        );
        ...
        assertGt(
            captured,
            0,
            "Hook should capture currency0 when pool overpays"
        );
        console.log("Captured amount:", captured);

        // Test oneForZero (currency1 -> currency0) - should NOT capture arbitrage
        console.log(
            "Testing oneForZero: currency1->currency0 (should NOT capture arbitrage)"
        );
        ...
        assertFalse(
            arbitrageCaptured,
            "ArbitrageCaptured event should NOT be emitted for oneForZero"
        );
    ...
    function test_RealisticETHUSDCScenario() public {
        ...
        MockERC20 token0 = MockERC20(
            Currency.unwrap(realisticPoolKey.currency0)
        );
        MockERC20 token1 = MockERC20(
            Currency.unwrap(realisticPoolKey.currency1)
        );
        bool ethIsCurrency0 = (token0.decimals() == 18 &&
            token1.decimals() == 6);
        ...
        SwapParams memory swapParams = SwapParams({
            ...
            amountSpecified: -int256(swapAmount),
            sqrtPriceLimitX96: shouldCaptureDirection
                ? TickMath.MIN_SQRT_PRICE + 1
                : TickMath.MAX_SQRT_PRICE - 1
        });
        ...
        console.log("Gas used for realistic ETH/USDC swap:", gasUsed);
        ...
        assertTrue(
            arbitrageCaptured,
            "Should detect arbitrage in realistic scenario"
        );
        assertTrue(
            captured0 > 0 || captured1 > 0,
            "Should capture some tokens"
        );
