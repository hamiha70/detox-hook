...
contract SwapRouterIntegrationTest is Test, Deployers {
    ...
    function test_PoolSwapTestDeployment() public {
        ...
        assertTrue(
            ...
            "PoolSwapTest should be deployed"
        );
        ...
        console.log(
            "[PASS] PoolSwapTest deployment and configuration verified"
        );
    }
    ...
    function test_TokenApprovalForPoolSwapTest() public {
        ...
        uint256 initialAllowance0 = MockERC20(Currency.unwrap(currency0))
            .allowance(swapper, address(swapRouter));
        uint256 initialAllowance1 = MockERC20(Currency.unwrap(currency1))
            .allowance(swapper, address(swapRouter));
        ...
        // Verify approvals
        uint256 newAllowance0 = MockERC20(Currency.unwrap(currency0)).allowance(
            swapper,
            address(swapRouter)
        );
        uint256 newAllowance1 = MockERC20(Currency.unwrap(currency1)).allowance(
            swapper,
            address(swapRouter)
        );
    ...
    function test_ExactInputSwapCurrency0ToCurrency1_NoHookData() public {
        ...
        uint256 balance0Before = MockERC20(Currency.unwrap(currency0))
            .balanceOf(swapper);
        uint256 balance1Before = MockERC20(Currency.unwrap(currency1))
            .balanceOf(swapper);
        ...
        uint256 balance0After = MockERC20(Currency.unwrap(currency0)).balanceOf(
            swapper
        );
        uint256 balance1After = MockERC20(Currency.unwrap(currency1)).balanceOf(
            swapper
        );

        assertTrue(
            balance0After < balance0Before,
            "Currency0 balance should decrease"
        );
        assertTrue(
            balance1After > balance1Before,
            "Currency1 balance should increase"
        );

        vm.stopPrank();

        console.log(
            "[PASS] Exact input currency0->currency1 swap without hook data successful"
        );
    ...
    function test_ExactInputSwapCurrency1ToCurrency0_WithApproval() public {
        ...
        assertTrue(
            balance0After > balance0Before,
            "Currency0 balance should increase"
        );
        assertTrue(
            balance1After < balance1Before,
            "Currency1 balance should decrease"
        );

        vm.stopPrank();

        console.log(
            "[PASS] Exact input currency1->currency0 swap with approval successful"
        );
    ...
    function test_SwapWithHookData() public {
        ...
        // Note: DetoxHook doesn't receive ETH, only processes token fees
        ...
        // Verify hook was called - DetoxHook processes token fees, not ETH
        // Note: DetoxHook doesn't hold ETH, only processes token fees through fee extraction

        vm.stopPrank();

        console.log("[PASS] Swap with hook data successful");
        console.log("Hook successfully processed the swap");
    }
    ...
    function test_GasOptimization_MultipleSwaps() public {
        ...
        assertTrue(
            gasUsed1 < 500000,
            "Currency0->currency1 swap gas usage too high"
        );
        assertTrue(
            gasUsed2 < 500000,
            "Currency1->currency0 swap gas usage too high"
        );
    }
    ...
    function test_HookMEVDetectionSimulation() public {
        ...
        // Record balances before swap (tracking token balances, not ETH)
        uint256 swapperBalance1Before = MockERC20(Currency.unwrap(currency1))
            .balanceOf(swapper);
        ...
        uint256 swapperBalance1After = MockERC20(Currency.unwrap(currency1))
            .balanceOf(swapper);
        assertTrue(
            swapperBalance1After > swapperBalance1Before,
            "Swapper should receive currency1"
        );

        vm.stopPrank();

        console.log("[PASS] Hook MEV detection simulation working");
        console.log("Hook successfully processed the MEV detection");
        console.log(
            "Currency1 received by swapper:",
            swapperBalance1After - swapperBalance1Before
        );
    }
    ...
    function test_SwapWithStalePrice() public {
        ...
        vm.stopPrank();
        console.log(
            "[PASS] Swap with stale price (should not interfere) successful"
        );
    }
    ...
    function test_SwapWithWideConfidence() public {
        ...
        vm.stopPrank();
        console.log(
            "[PASS] Swap with wide confidence (should not interfere) successful"
        );
    }
    ...
    function testEvent_ArbitrageCaptured() public {
        ...
        for (uint i = 0; i < entries.length; i++) {
            ...
            if (entries[i].topics.length > 2)
                console.logBytes32(entries[i].topics[2]);
            if (entries[i].topics.length > 3)
                console.logBytes32(entries[i].topics[3]);
    ...
    function _encodeMockPythUpdate(
        bytes32 priceId,
        uint64 timestamp,
        int64 price,
        uint64 conf,
    ...
    function _generateMockHookData() internal view returns (bytes memory) {
        ...
        bytes memory ethUpdate = _encodeMockPythUpdate(
            ETH_USD_PRICE_ID,
            uint64(block.timestamp),
            int64(2500e8),
            uint64(1e6),
        ...
        bytes memory usdcUpdate = _encodeMockPythUpdate(
            USDC_USD_PRICE_ID,
            uint64(block.timestamp),
            int64(1e8),
            uint64(1e4),
