...
contract InitializePoolsWithHook is Script {
    ...
    // Events
    event PoolInitializationStarted(
    ...
    event PoolInitializationCompleted(
    ...
    function _validateHookAndCheckBalances() internal {
        ...
        require(
            ...
            "Hook address must contain contract code"
        );
        ...
        if (!sufficientBalance) {
            ...
            revert(
                "Insufficient balance for pool operations. Please fund the deployer address."
            );
        } else {
    ...
    function _initializeContracts() internal {
        ...
        address poolManagerAddress = ChainAddresses.getPoolManager(
            block.chainid
        );
        address usdcAddress = ChainAddresses.getUSDC(block.chainid);
        ...
        address modifyLiquidityAddress = ChainAddresses
            .getPoolModifyLiquidityTest(block.chainid);
        require(
            modifyLiquidityAddress != address(0),
            "PoolModifyLiquidityTest address not found"
        );
        modifyLiquidityRouter = PoolModifyLiquidityTest(modifyLiquidityAddress);
        console.log(
            "  Modify Liquidity Router:",
        ...
        try hook.poolManager() returns (IPoolManager hookPoolManager) {
            require(
                address(hookPoolManager) == address(poolManager),
                "Hook pool manager mismatch"
            );
    ...
    function _initializePools() internal {
        ...
        try poolManager.initialize(poolKey1, SQRT_PRICE_2500) returns (
            int24 tick1
        ) {
        ...
        try poolManager.initialize(poolKey2, SQRT_PRICE_2600) returns (
            int24 tick2
        ) {
    ...
    function _logDeploymentSummary() internal view {
        ...
        console.log(
            "Hook Contract:",
            string.concat(explorerBase, "/address/", vm.toString(hookAddress))
        );
        console.log(
            "Pool Manager:",
            string.concat(
