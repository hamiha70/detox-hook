...
contract VerifyDeployment is Script {
    ...
    function _autoDetectDeployment() internal pure returns (DeploymentInfo memory) {
    ...
    function _verifyDeployment(DeploymentInfo memory deployment) internal view returns (VerificationResult memory) {
        ...
        result.priceRegistryValid = _verifyPriceRegistry(deployment.priceRegistry);
        
        // 3. Verify SwapRouterFixed
        result.swapRouterValid = _verifySwapRouterFixed(deployment.swapRouterFixed);
    ...
    function _verifyDetoxHook(address hookAddress) internal view returns (bool) {
        ...
        try DetoxHookV2(payable(hookAddress)).poolManager() returns (IPoolManager pm) {
    ...
    function _verifyPriceRegistry(address registryAddress) internal view returns (bool) {
    ...
    function _verifySwapRouterFixed(address routerAddress) internal view returns (bool) {
    ...
    function _verifyPoolManager(address pmAddress) internal view returns (bool) {
    ...
    function _verifyHookFlags(address hookAddress) internal pure returns (bool) {
    ...
    function _verifyPoolsInitialized(DeploymentInfo memory deployment) internal view returns (bool) {
        ...
        // Check pool 1
        (uint160 sqrtPriceX96, int24 tick, uint24 protocolFee, uint24 lpFee) = StateLibrary.getSlot0(pm, poolKey1.toId());
        if (sqrtPriceX96 != 0) {
        ...
        // Check pool 2
        (sqrtPriceX96, tick, protocolFee, lpFee) = StateLibrary.getSlot0(pm, poolKey2.toId());
        if (sqrtPriceX96 != 0) {
    ...
    function _verifyLiquidity(DeploymentInfo memory deployment) internal view returns (bool) {
        ...
        uint256 usdcBalance = MockERC20(deployment.mockUsdc).balanceOf(deployment.poolManager);
    ...
    function _displayVerificationResults(DeploymentInfo memory deployment, VerificationResult memory result) internal pure {
        console.log("");
        console.log("===============================================");
        ...
        console.log("DetoxHook Valid:", result.detoxHookValid ? "[PASS]" : "[FAIL]");
        console.log("PriceRegistry Valid:", result.priceRegistryValid ? "[PASS]" : "[FAIL]");
        console.log("SwapRouterFixed Valid:", result.swapRouterValid ? "[PASS]" : "[FAIL]");
        console.log("MockUSDC Valid:", result.mockUsdcValid ? "[PASS]" : "[FAIL]");
        console.log("PoolManager Valid:", result.poolManagerValid ? "[PASS]" : "[FAIL]");
        console.log("Hook Flags Valid:", result.hookFlagsValid ? "[PASS]" : "[FAIL]");
        console.log("Pools Initialized:", result.poolsInitialized ? "[PASS]" : "[FAIL]");
        console.log("Liquidity Present:", result.liquidityPresent ? "[PASS]" : "[WARNING]");
        console.log("");
        ...
        string memory explorerBase = ChainAddresses.getBlockExplorer(deployment.chainId);
        console.log("DetoxHook:", string(abi.encodePacked(explorerBase, "/address/", _addressToString(deployment.detoxHook))));
        console.log("PriceRegistry:", string(abi.encodePacked(explorerBase, "/address/", _addressToString(deployment.priceRegistry))));
        console.log("SwapRouterFixed:", string(abi.encodePacked(explorerBase, "/address/", _addressToString(deployment.swapRouterFixed))));
        console.log("MockUSDC:", string(abi.encodePacked(explorerBase, "/address/", _addressToString(deployment.mockUsdc))));
    }
