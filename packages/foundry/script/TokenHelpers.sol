...
library TokenHelpers {
    ...
    event MockUSDCDeployed(address indexed mockUSDC, address indexed owner, uint256 chainId);
    event AccountFunded(address indexed account, address indexed token, uint256 amount);
    ...
    function deployMockUSDC(
        ...
        SafetyChecks.validateETHBalance(deployer, 0.001 ether, "MockUSDC deployment");
    ...
    /// @return mockUSDC The deployed MockUSDC contract
    function deployStandardMockUSDC(address deployer) internal returns (MockUSDC mockUSDC) {
    ...
    function fundAccount(MockUSDC mockUSDC, address account, uint256 amount) internal {
    ...
    function fundAccounts(MockUSDC mockUSDC, address[] memory accounts, uint256 amount) internal {
    ...
    function fundDemoAccounts(MockUSDC mockUSDC, address[] memory additionalAccounts) internal {
    ...
    function getMockUSDC(address mockUSDCAddress) internal pure returns (MockUSDC mockUSDC) {
    ...
    function getTokenInterface(MockUSDC mockUSDC) internal pure returns (IERC20Minimal token) {
    ...
    function validateMockUSDC(MockUSDC mockUSDC, address expectedOwner) internal view {
        ...
        require(bytes(mockUSDC.symbol()).length > 0, "MockUSDC should have a symbol");
        require(bytes(mockUSDC.name()).length > 0, "MockUSDC should have a name");
    ...
    function logTokenStatus(MockUSDC mockUSDC, address[] memory accounts) internal view {
    ...
    function isValidMockUSDC(address tokenAddress) internal view returns (bool isValid) {
