#!/usr/bin/env python3
"""
Add Liquidity to POOL1 (ETH/USDC 0.3%)
========================================

This script adds liquidity to DetoxHook POOL1 (ETH/USDC 0.3% fee pool)
using the provide_liquidity.py script with predefined parameters.

Pool Details:
- Pool ID: 0x5e6967b5ca922ff1aa7f25521cfd03d9a59c17536caa09ba77ed0586c238d23f
- Pair: ETH/USDC
- Fee: 3000 (0.3%)
- Hook: DetoxHook

Usage:
    python3 add_liquidity_pool1.py [--dry-run] [--verbose] [--amount <amount>]
"""
...
class Pool1LiquidityProvider:
    """Adds liquidity to DetoxHook POOL1 (ETH/USDC 0.3%)."""
    
    # POOL1 configuration
    POOL1_ID = "0x5e6967b5ca922ff1aa7f25521cfd03d9a59c17536caa09ba77ed0586c238d23f"
    POOL1_DESCRIPTION = "ETH/USDC 0.3% fee pool (~2500 USDC/ETH)"
    
    # Default liquidity amounts (in USDC)
    DEFAULT_USDC_AMOUNT = 100  # 100 USDC
    ...
    def build_command(self, wallet_address: str, usdc_amount: Optional[float] = None, 
        ...
        if usdc_amount is None and eth_amount is None:
            usdc_amount = self.DEFAULT_USDC_AMOUNT
        ...
        if usdc_amount is not None:
            cmd.extend(["--usdc-amount", str(usdc_amount)])
    ...
    def display_pool_info(self):
        """Display information about POOL1."""
        print("🏊 DetoxHook POOL1 Information")
        print("=" * 40)
        print(f"📊 Description: {self.POOL1_DESCRIPTION}")
        print(f"🆔 Pool ID: {self.POOL1_ID}")
        print(f"💰 Pair: ETH/USDC")
        print(f"💸 Fee: 3000 (0.3%)")
        print(f"🎯 Tick Spacing: 60")
        print(f"🪝 Hook: DetoxHook")
        print(f"💱 Target Price: ~2500 USDC/ETH")
        print()
    ...
    def display_default_amounts(self):
        ...
        print("=" * 30)
        print(f"💵 USDC Amount: {self.DEFAULT_USDC_AMOUNT} USDC")
        print(f"🪙 ETH Amount: {self.DEFAULT_ETH_AMOUNT} ETH")
        print(f"💱 Total Value: ~${self.DEFAULT_USDC_AMOUNT + (self.DEFAULT_ETH_AMOUNT * 2500):.2f}")
        print()
    
    def run_liquidity_provision(self, wallet_address: str, usdc_amount: Optional[float] = None,
        ...
        # Build command
        cmd = self.build_command(wallet_address, usdc_amount, eth_amount, dry_run)
        
        # Display what we're doing
        print("🚀 Adding Liquidity to POOL1")
        print("=" * 30)
        print(f"👛 Wallet: {wallet_address}")
        print(f"🏊 Pool: {self.POOL1_ID}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add liquidity to DetoxHook POOL1 (ETH/USDC 0.3%)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                                    # Use default amounts with env wallet
  %(prog)s 0x1234...                         # Use specific wallet with default amounts
  %(prog)s --usdc-amount 50 --eth-amount 0.02  # Custom amounts
  %(prog)s --dry-run --verbose               # Dry run with details
  %(prog)s --amount 200                      # 200 USDC worth of liquidity
        """
    )
    ...
    parser.add_argument(
        '--usdc-amount',
        type=float,
        help='Amount of USDC to provide (default: 100)'
    )
    ...
    parser.add_argument(
        '--amount',
        type=float,
        help='Total amount in USDC to provide (will calculate ETH amount)'
    )
    ...
    # Create provider
    provider = Pool1LiquidityProvider(verbose=args.verbose)
    ...
    if args.amount is not None:
        # Calculate amounts based on total USDC value
        total_usdc = args.amount
        # Assume 50/50 split for simplicity (in reality, this would depend on current price)
        usdc_amount = total_usdc / 2
        eth_amount = (total_usdc / 2) / 2500  # Approximate ETH amount at $2500/ETH
